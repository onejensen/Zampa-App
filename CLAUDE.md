...
- **User history**: actions `call`, `directions` tracked per-user in the `userHistory` collection. Favorites come from the `favorites` collection. Viewed in the Profile > Historial screen.

### Firestore Collections

| Collection | Purpose |
|---|---|
| `users` | Auth profile + role + notificationPreferences |
...
- Feature views live in `Features/{Auth,Feed,Favorites,Merchant,Profile,Subscription}/`. Profile includes `HistoryView`, `DietaryPreferencesView`, `NotificationPreferencesView`.
...
- **Jetpack Compose Navigation**: `EatOutNavHost` (`ui/navigation/Navigation.kt`, package `com.sozolab.zampa`) owns all routes — `Auth`, `Main`, `MerchantSetup`, `LocationOnboarding`, `Stats`, `MenuDetail`, `NotificationPreferences`, `DietaryPreferences`, `History`. Package is `com.sozolab.zampa` (legacy `com.sozolab.eatout` references may still appear in build artifacts and the Play Store URL).
...
- Release SHA-1: `94:98:FC:BD:6E:B8:69:0C:DB:82:29:43:30:8B:1F:DE:FA:22:AC:BE`
...
**Release keystore:** Lives at `~/zampa-release.keystore` (outside repo). Alias: `zampa`. Validity: 10000 days. Configure via `ZAMPA_KEYSTORE_PATH` / `ZAMPA_KEYSTORE_PASS` / `ZAMPA_KEY_PASS` env vars or `local.properties`. `build.gradle.kts` reads these. **Losing the keystore means never being able to update the app on Play Store.**
